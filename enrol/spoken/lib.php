...
/**
 * spoken enrolment plugin implementation.
 * @author Petr Skoda
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class enrol_spoken_plugin extends enrol_plugin {
    ...
    public function get_info_icons(array $instances) {
        ...
        foreach ($instances as $instance) {
            if ($this->can_spoken_enrol($instance, false) !== true) {
                // User can not enrol himspoken.
                ...
                // would hide spoken-enrolment icons from guests.
                continue;
            }
        ...
        $icons = array();
        if ($nokey) {
            $icons[] = new pix_icon('withoutkey', get_string('pluginname', 'enrol_spoken'), 'enrol_spoken');
        }
        if ($key) {
            $icons[] = new pix_icon('withkey', get_string('pluginname', 'enrol_spoken'), 'enrol_spoken');
        }
        return $icons;
    }
    ...
    public function show_enrolme_link(stdClass $instance) {
        ...
        if (true !== $this->can_spoken_enrol($instance, false)) {
            return false;
        }
    ...
    public function can_add_instance($courseid) {
        ...
        if (!has_capability('moodle/course:enrolconfig', $context) or !has_capability('enrol/spoken:config', $context)) {
            return false;
        }
    ...
    public function enrol_page_hook(stdClass $instance) {
        ...
        $enrolstatus = $this->can_spoken_enrol($instance);

        if (true === $enrolstatus) {
            // This user can spoken enrol using this instance.
            $form = new enrol_spoken_enrol_form(null, $instance);
            $instanceid = optional_param('instance', 0, PARAM_INT);
            if ($instance->id == $instanceid) {
                if ($data = $form->get_data()) {
                    $this->enrol_spoken($instance, $data);
                }
            }
        } else {
            // This user can not spoken enrol using this instance. Using an empty form to keep
            // the UI consistent with other enrolment plugins that returns a form.
            $data = new stdClass();
            $data->header = $this->get_instance_name($instance);
            $data->info = $enrolstatus;

            // The can_spoken_enrol call returns a button to the login page if the user is a
            // guest, setting the login url to the form if that is the case.
            $url = isguestuser() ? get_login_url() : null;
            $form = new enrol_spoken_empty_form($url, $data);
        }

        // @custom create enrollment and redirect to course from here
        //
        if (!$enrol = enrol_get_plugin('spoken')) {
            return false;
        }
        ...
        if ($count) {
            $enrol->enrol_user($instance, $USER->id, 5, 0, 0); // 5 => role student role id
        } else {
            return get_string('canntenrol', 'enrol_spoken');
        }
        // @custom
    ...
    /**
     * Checks if user can spoken enrol.
     *
     * @param stdClass $instance enrolment instance
     ...
     */
    public function can_spoken_enrol(stdClass $instance, $checkuserenrolment = true) {
        ...
        if (!has_capability('enrol/spoken:enrolspoken', context_course::instance($instance->courseid))) {
            return get_string('canntenrol', 'enrol_spoken');
        }

        if ($instance->enrolstartdate != 0 and $instance->enrolstartdate > time()) {
            return get_string('canntenrolearly', 'enrol_spoken', userdate($instance->enrolstartdate));
        }

        if ($instance->enrolenddate != 0 and $instance->enrolenddate < time()) {
            return get_string('canntenrollate', 'enrol_spoken', userdate($instance->enrolenddate));
        }
        ...
        if ($instance->customint3 > 0) {
            ...
            if ($count >= $instance->customint3) {
                // Bad luck, no more spoken enrolments here.
                return get_string('maxenrolledreached', 'enrol_spoken');
            }
        }
        ...
        if ($instance->customint5) {
            ...
            if (!cohort_is_member($instance->customint5, $USER->id)) {
                ...
                return markdown_to_html(get_string('cohortnonmemberinfo', 'enrol_spoken', $a));
            }
        }

        // @sanmugam: We can write the spoken login here
        // print_r($instance);
        // return get_string('canntenrol', 'enrol_spoken');
        // die;
        return true;
    ...
    public function get_enrol_info(stdClass $instance) {
        ...
        $instanceinfo->status = $this->can_spoken_enrol($instance);

        if ($instance->password) {
            $instanceinfo->requiredparam = new stdClass();
            $instanceinfo->requiredparam->enrolpassword = get_string('password', 'enrol_spoken');
        }
        ...
        if ((true === $instanceinfo->status) && $instance->password) {
            $instanceinfo->wsfunction = 'enrol_spoken_get_instance_info';
        }
        return $instanceinfo;
    }
    ...
    protected function email_welcome_message($instance, $user) {
        ...
        if (trim($instance->customtext1) !== '') {
            ...
            $messagetext = get_string('welcometocoursetext', 'enrol_spoken', $a);
            $messagehtml = text_to_html($messagetext, null, false, true);
        }

        $subject = get_string('welcometocourse', 'enrol_spoken', format_string($course->fullname, true, array('context'=>$context)));
    ...
    public function sync(progress_trace $trace, $courseid = null) {
        ...
        $trace->output('Verifying spoken-enrolments...');
        ...
        // Note: the logic of spoken enrolment guarantees that user logged in at least once (=== u.lastaccess set)
        ...
        $sql = "SELECT e.*, ue.userid
                  ...
                  JOIN {enrol} e ON (e.id = ue.enrolid AND e.enrol = 'spoken' AND e.customint2 > 0)
        ...
        $trace->output('...user spoken-enrolment updates finished.');
        $trace->finished();
    ...
    /**
     * Returns the user who is responsible for spoken enrolments in given instance.
     *
     * Usually it is the first editing teacher - the person with "highest authority"
     * as defined by sort_by_roleassignment_authority() having 'enrol/spoken:manage'
     * capability.
     *
    ...
    protected function get_enroller($instanceid) {
        ...
        if ($users = get_enrolled_users($context, 'enrol/spoken:manage')) {
        ...
        if ($merge and $instances = $DB->get_records('enrol', $merge, 'id')) {
            ...
            if (!empty($data->customint5)) {
                if ($step->get_task()->is_samesite()) {
                    ...
                    // Use some id that can not exist in order to prevent spoken enrolment,
    ...
    public function restore_role_assignment($instance, $roleid, $userid, $contextid) {
        ...
        // we do not use component in manual or spoken enrol.
    ...
    public function can_delete_instance($instance) {
        $context = context_course::instance($instance->courseid);
        return has_capability('enrol/spoken:config', $context);
    }
    ...
    public function can_hide_show_instance($instance) {
        ...
        if (!has_capability('enrol/spoken:config', $context)) {
            return false;
        }
    ...
    protected function get_expirynotify_options() {
        $options = array(0 => get_string('no'),
                         1 => get_string('expirynotifyenroller', 'enrol_spoken'),
                         2 => get_string('expirynotifyall', 'enrol_spoken'));
        return $options;
    }
    ...
    /**
     * The spoken enrollment plugin has several bulk operations that can be performed.
    ...
    public function get_bulk_operations(course_enrolment_manager $manager) {
        ...
        if (has_capability("enrol/spoken:manage", $context)) {
            $bulkoperations['editselectedusers'] = new enrol_spoken_editselectedusers_operation($manager, $this);
        }
        if (has_capability("enrol/spoken:unenrol", $context)) {
            $bulkoperations['deleteselectedusers'] = new enrol_spoken_deleteselectedusers_operation($manager, $this);
        }
    ...
    public function edit_instance_form($instance, MoodleQuickForm $mform, $context) {
        ...
        $mform->addElement('select', 'status', get_string('status', 'enrol_spoken'), $options);
        $mform->addHelpButton('status', 'status', 'enrol_spoken');

        $options = $this->get_newenrols_options();
        $mform->addElement('select', 'customint6', get_string('newenrols', 'enrol_spoken'), $options);
        $mform->addHelpButton('customint6', 'newenrols', 'enrol_spoken');
        $mform->disabledIf('customint6', 'status', 'eq', ENROL_INSTANCE_DISABLED);

        $passattribs = array('size' => '20', 'maxlength' => '50');
        $mform->addElement('passwordunmask', 'password', get_string('password', 'enrol_spoken'), $passattribs);
        $mform->addHelpButton('password', 'password', 'enrol_spoken');
        ...
        $mform->addElement('select', 'customint1', get_string('groupkey', 'enrol_spoken'), $options);
        $mform->addHelpButton('customint1', 'groupkey', 'enrol_spoken');

        $roles = $this->extend_assignable_roles($context, $instance->roleid);
        $mform->addElement('select', 'roleid', get_string('role', 'enrol_spoken'), $roles);

        $options = array('optional' => true, 'defaultunit' => 86400);
        $mform->addElement('duration', 'enrolperiod', get_string('enrolperiod', 'enrol_spoken'), $options);
        $mform->addHelpButton('enrolperiod', 'enrolperiod', 'enrol_spoken');
        ...
        $mform->addElement('date_time_selector', 'enrolstartdate', get_string('enrolstartdate', 'enrol_spoken'), $options);
        $mform->setDefault('enrolstartdate', 0);
        $mform->addHelpButton('enrolstartdate', 'enrolstartdate', 'enrol_spoken');

        $options = array('optional' => true);
        $mform->addElement('date_time_selector', 'enrolenddate', get_string('enrolenddate', 'enrol_spoken'), $options);
        $mform->setDefault('enrolenddate', 0);
        $mform->addHelpButton('enrolenddate', 'enrolenddate', 'enrol_spoken');

        $options = $this->get_longtimenosee_options();
        $mform->addElement('select', 'customint2', get_string('longtimenosee', 'enrol_spoken'), $options);
        $mform->addHelpButton('customint2', 'longtimenosee', 'enrol_spoken');

        $mform->addElement('text', 'customint3', get_string('maxenrolled', 'enrol_spoken'));
        $mform->addHelpButton('customint3', 'maxenrolled', 'enrol_spoken');
        ...
        if (count($cohorts) > 1) {
            $mform->addElement('select', 'customint5', get_string('cohortonly', 'enrol_spoken'), $cohorts);
            $mform->addHelpButton('customint5', 'cohortonly', 'enrol_spoken');
        } else {
        ...
        $mform->addElement('select', 'customint4', get_string('sendcoursewelcomemessage', 'enrol_spoken'),
                enrol_send_welcome_email_options());
        $mform->addHelpButton('customint4', 'sendcoursewelcomemessage', 'enrol_spoken');

        $options = array('cols' => '60', 'rows' => '8');
        $mform->addElement('textarea', 'customtext1', get_string('customwelcomemessage', 'enrol_spoken'), $options);
        $mform->addHelpButton('customtext1', 'customwelcomemessage', 'enrol_spoken');

        if (enrol_accessing_via_instance($instance)) {
            $warntext = get_string('instanceeditspokenwarningtext', 'core_enrol');
            $mform->addElement('static', 'spokenwarn', get_string('instanceeditspokenwarning', 'core_enrol'), $warntext);
        }
    }
    ...
    public function edit_instance_validation($data, $files, $instance, $context) {
        ...
        if ($data['status'] == ENROL_INSTANCE_ENABLED) {
            if (!empty($data['enrolenddate']) and $data['enrolenddate'] < $data['enrolstartdate']) {
                $errors['enrolenddate'] = get_string('enrolenddaterror', 'enrol_spoken');
            }
        }
    ...
    /**
     * Gets a list of roles that this user can assign for the course as the default for spoken-enrolment.
     *
     * @param context $context the context.
     * @param integer $defaultrole the id of the role that is set as the default for spoken-enrolment
    ...
    public function get_welcome_email_contact($sendoption, $context) {
        ...
        if ($sendoption == ENROL_SEND_EMAIL_FROM_COURSE_CONTACT) {
            ...
            // Send as the first user with enrol/spoken:holdkey capability assigned in the course.
            list($sort) = users_order_by_sql('u');
            $keyholders = get_users_by_capability($context, 'enrol/spoken:holdkey', 'u.*', $sort);
            if (!empty($keyholders)) {
 ...
 */
function enrol_spoken_get_fontawesome_icon_map() {
    return [
        'enrol_spoken:withkey' => 'fa-key',
        'enrol_spoken:withoutkey' => 'fa-sign-in',
    ];
}
