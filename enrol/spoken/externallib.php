...
/**
 * spoken enrol plugin external functions
 *
...
/**
 * spoken enrolment external functions.
 *
 * @package   enrol_spoken
 ...
 * @since     Moodle 2.6
 */
class enrol_spoken_external extends external_api {
    ...
    public static function get_instance_info_parameters() {
        return new external_function_parameters(
                array('instanceid' => new external_value(PARAM_INT, 'instance id of spoken enrolment plugin.'))
            );
    }

    /**
     * Return spoken-enrolment instance information.
     *
     * @param int $instanceid instance id of spoken enrolment plugin.
    ...
    public static function get_instance_info($instanceid) {
        ...
        // Retrieve spoken enrolment plugin.
        $enrolplugin = enrol_get_plugin('spoken');
    ...
    public static function enrol_user_parameters() {
        return new external_function_parameters(
            array(
                ...
                'instanceid' => new external_value(PARAM_INT, 'Instance id of spoken enrolment plugin.', VALUE_DEFAULT, 0)
            )
        );
    }

    /**
     * spoken enrol the current user in the given course.
     *
    ...
    public static function enrol_user($courseid, $password = '', $instanceid = 0) {
        ...
        // Retrieve the spoken enrolment plugin.
        $enrol = enrol_get_plugin('spoken');
        if (empty($enrol)) {
            throw new moodle_exception('canntenrol', 'enrol_spoken');
        }

        // We can expect multiple spoken-enrolment instances.
        $instances = array();
        $enrolinstances = enrol_get_instances($course->id, true);
        foreach ($enrolinstances as $courseenrolinstance) {
            if ($courseenrolinstance->enrol == "spoken") {
                // Instance specified.
        ...
        foreach ($instances as $instance) {
            $enrolstatus = $enrol->can_spoken_enrol($instance);
            if ($enrolstatus === true) {
                if ($instance->password and $params['password'] !== $instance->password) {
                    ...
                    if ($instance->customint1) {
                        ...
                        if (!enrol_spoken_check_group_enrolment_key($course->id, $params['password'])) {
                            $warnings[] = array(
                                'item' => 'instance',
                                'itemid' => $instance->id,
                                'warningcode' => '2',
                                'message' => get_string('passwordinvalid', 'enrol_spoken')
                            );
                            continue;
                        }
                    } else {
                        if ($enrol->get_config('showhint')) {
                            ...
                            $warnings[] = array(
                                ...
                                'itemid' => $instance->id,
                                'warningcode' => '3',
                                'message' => s(get_string('passwordinvalidhint', 'enrol_spoken', $hint)) // message is PARAM_TEXT.
                            );
                            continue;
                        } else {
                            $warnings[] = array(
                ...
                $enrol->enrol_spoken($instance, (object) $data);
                $enrolled = true;
                break;
            } else {
                $warnings[] = array(
